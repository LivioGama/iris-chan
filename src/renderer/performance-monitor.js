import { metric as logMetric } from './logger.js';
...
function pickRendererHeap(perfMemory) {
	if (!perfMemory || typeof perfMemory !== 'object') return null;
	return {
...
export class PerformanceMonitor {
	...
	recordFrame({ deltaMs, avatarVisible = true, documentHidden = false } = {}) {
		...
		if (this._renderWindow.length < RENDER_SAMPLE_WINDOW) return;
	...
	cancelVoiceTurn(turn) {
		if (!turn) return;
		if (this._report.voice.status.activeTurnId !== turn.id) return;
		this._report.voice.status.idle = true;
		this._report.voice.status.activeTurnId = null;
	}

	completeVoiceTurn(turn) {
		if (!turn || !turn.captureEndedAt || !turn.requestStartedAt) return null;
		const sample = {
			recordedAt: new Date().toISOString(),
			turnId: turn.id,
		...
		logMetric('VoiceBenchmark', sample);
		return sample;
	}
	...
	recordResourceSample(sample = {}) {
		...
		logMetric('ResourceBenchmark', next);
	}
	...
	_scheduleResourceSample(delayMs = RESOURCE_SAMPLE_INTERVAL_MS) {
		if (this._resourceTimer) clearTimeout(this._resourceTimer);
	...
	_scheduleIpcSample(delayMs = IPC_SAMPLE_INTERVAL_MS) {
		if (this._ipcTimer) clearTimeout(this._ipcTimer);
	...
	async _sampleIpc() {
		...
		try {
			...
			if (sample) this.recordIpcSample(sample);
		} catch {}
