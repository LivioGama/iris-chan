...
exports.BargeInDetector = void 0;
const DEFAULTS = {
...
class BargeInDetector {
    ...
    bufferChunk(chunk, now = Date.now()) {
        if (!this._active || this._confirmed || !chunk)
            return;
    ...
    _requiredMicVolume(playbackVolume, now = Date.now()) {
        ...
        return Math.max(this.options.activationThreshold, minRespondingThreshold, playbackVolume * playbackDominanceRatio, this._noiseFloor * this.options.noiseFloorMultiplier + this.options.noiseFloorOffset);
    }
    ...
    _updateNoiseFloor(micVolume) {
        ...
        this._noiseFloor = Math.max(this.options.activationThreshold, (1 - alpha) * this._noiseFloor + alpha * sample);
    }
    _trimBuffer(now) {
...
exports.BargeInDetector = BargeInDetector;
