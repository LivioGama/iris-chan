...
class AudioPlayback extends emitter_js_1.Emitter {
    constructor() {
        super();
        this.ctx = null;
        this.analyser = null;
    ...
    async init() {
        ...
        if (this.ctx && this.gainNode && this.analyser)
            return;
    ...
    async _setOutputDevice(preferredName) {
        if (!this.ctx?.setSinkId) {
            (0, logger_js_1.info)('Playback', 'setSinkId not supported, using system default');
            return;
        }
        try {
            const devices = await navigator.mediaDevices.enumerateDevices();
            const outputs = devices.filter(d => d.kind === 'audiooutput');
            (0, logger_js_1.info)('Playback', `Available outputs: ${outputs.map(d => d.label).join(', ')}`);
            ...
            if (name) {
                ...
                if (match) {
                    ...
                    (0, logger_js_1.info)('Playback', `Output device set to: ${match.label}`);
                    return;
                }
                (0, logger_js_1.error)('Playback', `Output device "${name}" not found, using default`);
            }
        }
        catch (err) {
            (0, logger_js_1.error)('Playback', `Failed to set output device: ${err.message}`);
        }
    }
    async setOutputDevice(name) {
        this._preferredOutput = name;
        if (this.ctx)
            await this._setOutputDevice(name);
    }
