...
exports.formatToolResponseText = formatToolResponseText;
exports.shouldRefreshScreenAfterTool = shouldRefreshScreenAfterTool;
exports.shouldDeferForegroundUiTool = shouldDeferForegroundUiTool;
exports.createToolCallHandler = createToolCallHandler;
const event_types_web_js_1 = require("../../shared/event-types.web.js");
const tool_log_js_1 = require("../ui/tool-log.js");
...
function isSearchTool(name, args) {
    if (name === 'web_search' || name === 'ask_chatgpt' || name === 'research')
        return true;
    if (name === 'run_terminal_command') {
        const cmd = (args?.command || '').toLowerCase();
        if (cmd.includes('search.py') || cmd.includes('perplexity'))
            return true;
    }
...
function searchLabel(name, args) {
    ...
    if (name === 'ask_chatgpt')
        return args?.prompt || 'Asking ChatGPT...';
    if (name === 'research')
        return args?.query || 'Deep researching...';
    if (name === 'run_terminal_command') {
...
function formatToolResponseText(result) {
...
function shouldRefreshScreenAfterTool(name, result = null) {
    if (name === 'run_ui_task') {
        return result?.ok === false;
    }
    return SCREEN_REFRESH_TOOLS.has(name);
}
function shouldDeferForegroundUiTool(name, { userSpeaking = false } = {}) {
...
function isNavigationalUiIntent(intentText = '') {
    ...
    if (!text)
        return false;
    if (DESTRUCTIVE_UI_INTENT_PATTERN.test(text))
        return false;
    return NAVIGATIONAL_UI_INTENT_PATTERN.test(text);
}
function createToolCallHandler({ gemini, onStateChange, onEvent, screen, getLastUserIntent, getSelfFixContext, onSelfFixAccepted, onSelfFixIntentPreamble, }) {
    ...
    function updateToolPresence(name, args, index, total) {
        const { label, detail } = (0, tool_log_js_1.getToolDisplay)(name, args);
        const step = total > 1 ? `Step ${index + 1} of ${total}` : 'In progress';
        (0, presence_indicator_js_1.setPresence)('tool', 'tool', {
            title: 'Working',
    ...
    function rejectDuplicateUiTask(call, message = SAME_TURN_UI_TASK_MESSAGE) {
        (0, logger_js_1.info)('Tool', `Suppressing duplicate ${call.name} for speech turn ${speechGeneration}`);
    ...
    function rejectPointerRetryBatch(calls, message = SAME_TURN_POINTER_RETRY_MESSAGE) {
        (0, logger_js_1.info)('Tool', `Suppressing repeated pointer retry batch for speech turn ${speechGeneration}`);
        for (const call of calls) {
            gemini.sendToolResponse(call.id, call.name, message);
        }
    }
    function attachPointerCaptureId(name, args) {
        if (!POINTER_TOOLS.has(name))
            return args || {};
    ...
    const _executeOne = async (name, args, id, index, total, options = {}) => {
        ...
        (0, tool_log_js_1.showToolStart)(dispatchName, toolArgs, index, total);
        (0, logger_js_1.info)('Tool', `Executing: ${dispatchName}(${JSON.stringify(toolArgs || {})})`.slice(0, 500));
        ...
        try {
            ...
            if (allowAutoEscalation && (0, interaction_policy_js_1.shouldAutoEscalateFromToolFailure)({
                toolName: dispatchName,
                result,
                intentText: escalationGoal,
            })) {
                const failureReason = result?.result || 'unknown failure';
                (0, logger_js_1.info)('Tool', `Auto-escalating ${dispatchName} → run_ui_task. goal="${escalationGoal}" reason="${failureReason}"`);
                ...
                (0, logger_js_1.info)('Tool', `Auto-escalation outcome: source=${name} escalated=${escalatedResult.ok !== false ? 'OK' : 'FAIL'} result=${escalatedText.slice(0, 300)}`);
                result = {
                    ...escalatedResult,
            ...
            (0, tool_log_js_1.showToolDone)(dispatchName, index, result.ok !== false);
            (0, workspace_bar_js_1.updateIfWorkspaceTool)(dispatchName);
            (0, logger_js_1.info)('Tool', `Result: ${dispatchName} → ${result.ok !== false ? 'OK' : 'FAIL'}: ${toolResponseText.slice(0, 300)}`);
            ...
            (0, tool_log_js_1.showToolDone)(dispatchName, index, false);
            (0, logger_js_1.error)('Tool', `Error: ${dispatchName} → ${err.message}`);
            ...
            if (activeToolCount === 0) {
                (0, presence_indicator_js_1.clearPresence)('tool');
            }
        }
    };
    const _executeDeferredUiCall = async (call) => {
        onEvent(event_types_web_js_1.EVENT_TYPES.TOOL_START, { tools: [call.name] });
        if (isUiTaskLockedForCurrentSpeech(call.name)) {
            rejectDuplicateUiTask(call);
            onEvent(event_types_web_js_1.EVENT_TYPES.TOOL_END, { tools: [call.name] });
            return;
        }
    ...
    function schedulePendingUiFlush() {
        clearPendingUiFlushTimer();
        if (!pendingUiCall || userSpeaking)
            return;
        pendingUiFlushTimer = setTimeout(() => {
            const call = pendingUiCall;
            pendingUiCall = null;
            pendingUiFlushTimer = null;
            if (!call || userSpeaking)
                return;
            _executeDeferredUiCall(call).catch((err) => {
                (0, logger_js_1.error)('Tool', `Deferred UI tool ${call.name} dispatch error: ${err.message}`);
            });
    ...
    function queueDeferredUiCall(call) {
        ...
        pendingUiCall = call;
        clearPendingUiFlushTimer();
        (0, logger_js_1.info)('Tool', `Deferring ${call.name} while user is still speaking`);
    }
    function rejectSuppressedToolCalls(calls, message = 'Ignored tool call from a suppressed model turn') {
        (0, logger_js_1.info)('Tool', `Suppressing ${calls.length} tool call(s) because the current model turn is not actionable`);
        for (const call of calls) {
            gemini.sendToolResponse(call.id, call.name, message);
        }
    }
    function shouldRejectSelfFixCall(call) {
        if (call?.name !== 'self_fix')
            return null;
    ...
    function rejectRoutingFailure(calls, message) {
        (0, logger_js_1.info)('Tool', `Rejecting ${calls.length} tool call(s) because routing failed: ${message}`);
        for (const call of calls) {
    ...
    const _processToolCalls = async (calls) => {
        ...
        onEvent(event_types_web_js_1.EVENT_TYPES.TOOL_START, { tools: executableCalls.map(c => c.name) });
        ...
        if (bgCalls.length > 0) {
            (0, logger_js_1.info)('Tool', `Dispatching ${bgCalls.length} background tool(s) without blocking voice`);
            for (let i = 0; i < bgCalls.length; i++) {
                ...
                _executeOne(name, args, id, i, bgCalls.length, { responseName: __responseName || name }).catch(err => {
                    (0, logger_js_1.error)('Tool', `Background tool ${name} dispatch error: ${err.message}`);
                });
            }
        }
        ...
        onEvent(event_types_web_js_1.EVENT_TYPES.TOOL_END, { tools: executableCalls.map(c => c.name) });
    };
