...
class BehaviorEngine {
    ...
    setState(nextState = {}) {
        if (!nextState || typeof nextState !== 'object')
            return false;
        if (!this.setMode(nextState.mode))
            return false;
    ...
    sanitize(text = '') {
        ...
        if (!clean || this.isBannedIdleText(clean))
            return '';
        const now = Date.now();
    ...
    canSpeak({ directReply = false, majorMilestone = false, selfFixAck = false } = {}) {
        if (selfFixAck)
            return !this.idleAckSent;
        if (directReply)
            return true;
        if (majorMilestone)
            return this.state.mode !== 'silent' || this.userSpokeSinceAssistant;
    ...
    isAwaitingSelfFixDetails() {
        if (!this.awaitingSelfFixDetails)
            return false;
    ...
    shouldEvaluateProactively({ frontmostApp = '', contextFingerprint = '', captureAgeMs = 0, now = Date.now() } = {}) {
        if (this.state.mode === 'silent')
            return false;
        if (!String(frontmostApp || '').trim())
            return false;
        if (!String(contextFingerprint || '').trim())
            return false;
        if (this.state.mode === 'passive' && now - this.lastUserActivityAt > this.proactiveUserActiveWindowMs)
            return false;
        if (!Number.isFinite(captureAgeMs) || captureAgeMs > 20_000)
            return false;
    ...
    canSuggest({ confidence = 0, contextFingerprint = '', now = Date.now() } = {}) {
        ...
        if (now - this.proactiveLastAt < cooldownMs)
            return false;
        if (this.state.mode === 'passive' && now - this.lastUserActivityAt > this.proactiveUserActiveWindowMs)
            return false;
        if (contextFingerprint && contextFingerprint === this.lastProactiveFingerprint)
            return false;
    ...
    canSpeakPassive(timeSinceUserMs) {
        if (this.idleResponseSent)
            return false;
        if (this.state.mode === 'silent')
            return false;
        if (timeSinceUserMs > 60000)
            return false;
        return true;
    }
