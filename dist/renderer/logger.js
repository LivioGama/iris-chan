...
function shouldConsole(level, tag) {
    if (!currentSettings.console.enabled)
        return false;
    if (!currentSettings.sources.rendererConsole)
        return false;
    if (rank(level) < rank(currentSettings.console.level))
        return false;
...
function metric(tag, payload, { level = 'info', flushMs = 5000 } = {}) {
