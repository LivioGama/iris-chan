...
exports.pushTimelineEvent = pushTimelineEvent;
exports.clearTimeline = clearTimeline;
const MAX_EVENTS = 30;
function pushTimelineEvent(evt) {
    if (!evt?.type)
        return;
...
function phaseFromType(type) {
    ...
    if (type === 'TOOL_START' || type === 'TOOL_END')
        return 'tool';
    if (type === 'ACTION_VERIFY_OK' || type === 'ACTION_VERIFY_FAIL')
        return 'verify';
    if (type === 'DB_HEALTH')
        return 'db';
    if (type === 'INTERRUPT')
...
function formatMessage(evt) {
    ...
    if (evt.type === 'INTERRUPT')
        return evt.payload?.reason || 'Work paused by the user.';
    ...
    if (evt.type === 'ACTION_VERIFY_FAIL')
        return 'Visual check needs another pass.';
