...
exports.getToolDisplay = getToolDisplay;
exports.showToolStart = showToolStart;
exports.showToolDone = showToolDone;
exports.hideToolLog = hideToolLog;
let container = null;
let hideTimer = null;
const TOOL_LABELS = {
...
function formatArgs(name, args) {
    if (!args)
        return '';
    if (name === 'open_app' && args.name)
        return args.name;
    if (name === 'get_default_app' && args.kind)
        return args.kind;
    if (name === 'run_ui_task' && args.goal)
        return args.goal.length > 60 ? args.goal.slice(0, 60) + '…' : args.goal;
    ...
    if (name === 'web_search' && args.query)
        return `"${args.query}"`;
    if (name === 'ask_chatgpt' && args.prompt)
        return args.prompt.length > 40 ? args.prompt.slice(0, 40) + '…' : args.prompt;
    if (name === 'run_terminal_command' && args.command)
        return args.command.length > 120 ? args.command.slice(0, 120) + '…' : args.command;
    if (name === 'type_text' && args.text)
        return args.text.length > 30 ? args.text.slice(0, 30) + '…' : args.text;
    if (name === 'press_key' && args.key)
        return args.key;
    if (name === 'use_skill' && args.skill_name)
        return args.skill_name;
    if (name === 'notify' && args.message)
        return args.message.length > 40 ? args.message.slice(0, 40) + '…' : args.message;
    if (name === 'move_file')
        return [args.from, args.to].filter(Boolean).map(p => p.split('/').pop()).join(' → ');
    if (name === 'window_manage' && args.action)
        return args.action;
...
function showToolStart(name, args, index, total) {
...
function showToolDone(name, index, ok) {
    const el = getContainer();
