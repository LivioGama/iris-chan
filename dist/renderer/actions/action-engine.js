...
class ActionEngine {
    ...
    async executeWithVerification(action, { perform, verify, maxRetries = 3, delayMs = 200 } = {}) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (ok) {
                this.eventBus.emitEvent(event_types_web_js_1.EVENT_TYPES.ACTION_VERIFY_OK, { action, attempt }, 'action-engine');
                return { ok: true, attempt };
            }
            lastError = new Error(`Verification failed at attempt ${attempt}`);
            this.eventBus.emitEvent(event_types_web_js_1.EVENT_TYPES.ACTION_VERIFY_FAIL, { action, attempt }, 'action-engine');
        }
