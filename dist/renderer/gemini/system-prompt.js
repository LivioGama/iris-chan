...
exports.refreshVocabulary = refreshVocabulary;
exports.buildPrioritizedVocab = buildPrioritizedVocab;
exports.buildCorrectionsPrompt = buildCorrectionsPrompt;
...
function buildPrioritizedVocab() {
    ...
    for (const { term } of ranked) {
        if (result.length >= MAX_SYSTEM_TERMS)
            break;
        result.push(term);
...
${(0, interaction_policy_js_1.buildInteractionPromptPolicy)({ mode, directMode, feedbackEnabled, introversionEnabled })}`;
...
${buildPrioritizedVocab().map(t => `\u2022 ${t}`).join('\n') || '(none configured)'}${buildCorrectionsPrompt()}${(0, recent_seen_store_js_1.buildRecentSeenPrompt)()}`;
}
