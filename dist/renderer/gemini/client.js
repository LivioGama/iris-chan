...
exports.classifySelfFixRequest = classifySelfFixRequest;
// WebSocket lifecycle + message parsing (no tool schemas)
const emitter_js_1 = require("../../shared/emitter.js");
const tool_declarations_js_1 = require("./tool-declarations.js");
...
function splitManagementTaskIds(value = '') {
    return Array.from(new Set(Array.from(String(value || '').matchAll(MANAGEMENT_TASK_ID_PATTERN))
...
function parseManagementCorrections(text = '') {
    ...
    if (!signalsManagementCorrection)
        return null;
    ...
    for (const match of rawText.matchAll(/([a-z][a-z0-9_-]*eng)\s*\(([^)]+)\)/ig)) {
        ...
        if (!agent || !taskIds.length)
            continue;
        approvals.push({ agent, taskIds });
    }
    if (!cancellations.length && !approvals.length)
        return null;
    return {
        cancellations,
        approvals,
...
function buildManagementCorrectionsNote(parsed) {
    ...
    if (parsed.preserveExecutionCritical)
        lines.push('- preserve execution-critical tasks');
    if (parsed.prioritizeP0)
        lines.push('- prioritize p0 planning lanes, including memory/safety/research-observability coverage');
    if (parsed.requireExactSequence)
        lines.push('- execute first-wave approvals exactly as listed');
...
function classifySelfFixRequest(text = '', { awaitingDetails = false } = {}) {
    ...
    const detailSignals = SELF_FIX_DETAIL_PATTERNS.reduce((count, pattern) => count + (pattern.test(normalized) ? 1 : 0), 0);
...
class GeminiClient extends emitter_js_1.Emitter {
    constructor() {
        super();
        this.ws = null;
        this.apiKey = null;
    ...
    async _loadSetupResources() {
        const [vocabRefresh, skillDeclarations, skillPrompts, skillCatalog,] = await Promise.allSettled([
            (0, system_prompt_js_1.refreshVocabulary)(),
        ...
        if (vocabRefresh.status === 'rejected') {
            (0, logger_js_1.warn)('Gemini', `Vocabulary refresh failed before connect: ${vocabRefresh.reason?.message || vocabRefresh.reason || 'unknown error'}`);
        }
    ...
    _getSystemInstruction() {
        ...
        const nextInstruction = `${(0, system_prompt_js_1.buildSystemInstruction)({
            behaviorState: this._behaviorState,
        })}${this._cachedSkillSection}`;
    ...
    _connect() {
        ...
        (0, logger_js_1.info)('Gemini', `Connecting to ${ENDPOINT}...`);
        ...
        this.ws.onmessage = async (ev) => {
            ...
            try {
                ...
                (0, logger_js_1.error)('Gemini', 'Parse error:', e);
            }
        };
        this.ws.onerror = (err) => {
            if (id !== this._connectId)
            ...
            (0, logger_js_1.error)('Gemini', 'WebSocket error:', errorMsg);
            this.emit('error', err);
        };
        this.ws.onclose = (ev) => {
            if (id !== this._connectId)
                return; // stale WS — don't reconnect
            this.connected = false;
            this.sessionReady = false;
            (0, logger_js_1.info)('Gemini', `WebSocket closed: code=${ev.code}, reason=${ev.reason}`);
    ...
    _getSetupPayloadBundle() {
        ...
        do {
            ...
            nextLevel += 1;
            (0, logger_js_1.warn)('Gemini', `Setup payload estimated at ${serialized.length} chars; degrading to ${this._getSetupFallbackProfile(nextLevel).label} profile before connect`);
        } while (true);
    ...
    _buildSetupPayload(fallbackProfile = this._getSetupFallbackProfile()) {
        ...
        const functionDeclarations = [
            ...tool_declarations_js_1.toolDeclarations,
    ...
    _getTrimmedSystemInstruction(profile = this._getSetupFallbackProfile()) {
        ...
        if (fullInstruction.length <= budget)
            return fullInstruction;
        const trimmed = `${fullInstruction.slice(0, Math.max(0, budget - 29)).trimEnd()}\n\n[system instruction truncated]`;
        (0, logger_js_1.warn)('Gemini', `Trimmed system instruction from ${fullInstruction.length} to ${trimmed.length} chars for ${profile?.label || 'unknown'} setup profile`);
        return trimmed;
    }
    ...
    _applyInvalidArgumentFallback(reason = '') {
        if (this._setupFallbackLevel >= SETUP_FALLBACK_PROFILES.length - 1) {
            (0, logger_js_1.warn)('Gemini', `Invalid-argument close persisted after all setup fallbacks. reason=${reason || 'unknown'}`);
            return false;
        }
        this._setupFallbackLevel += 1;
        this._invalidateSetupCache();
        const profile = this._getSetupFallbackProfile();
        (0, logger_js_1.warn)('Gemini', `Invalid-argument close detected. Retrying with ${profile.label} setup profile. reason=${reason || 'unknown'}`);
        return true;
    }
    _buildSkillSection() {
        let section = '';
        ...
        if (truncated) {
            (0, logger_js_1.warn)('Gemini', `Trimmed skill prompt/catalog section to ${section.length} chars to keep setup payload stable`);
        }
        return section;
    }
    _handleMessage(msg) {
        if (msg.setupComplete) {
            ...
            if (this._setupFallbackLevel > 0) {
                const profile = this._getSetupFallbackProfile();
                (0, logger_js_1.info)('Gemini', `Session ready using ${profile.label} setup fallback`);
            }
            this.emit('ready');
            return;
        }
    ...
    _enqueueInboundMessage(raw) {
        ...
        if (this._inboundDrainScheduled)
            return;
    ...
    sendToolResponse(callId, name, result) {
        ...
        if (!this.connected) {
            (0, logger_js_1.error)('Gemini', `sendToolResponse for "${name}" but WS not connected — response will be dropped`);
        }
        const resultText = typeof result === 'string' ? result : JSON.stringify(result);
        this._send(`{"toolResponse":{"functionResponses":[{"id":${JSON.stringify(callId)},"name":${JSON.stringify(name)},"response":{"result":${JSON.stringify(resultText)}}}]}}`);
    }
    sendText(text) {
        if (!this.sessionReady)
            return;
        ...
        this._send(`{"clientContent":{"turns":[{"role":"user","parts":[{"text":${JSON.stringify(outgoingText)}}]}],"turnComplete":true}}`);
    }
    sendRealtimeText(text) {
        if (!this.sessionReady || !text)
            return;
    ...
    async sendVocabUpdate() {
        if (!this.sessionReady)
            return;
        await (0, system_prompt_js_1.refreshVocabulary)();
        const terms = (0, system_prompt_js_1.buildPrioritizedVocab)();
        const corrections = (0, system_prompt_js_1.buildCorrectionsPrompt)();
        const recentSeen = (0, recent_seen_store_js_1.buildRecentSeenPrompt)();
    ...
    _tryReconnect() {
        ...
        this.retryCount++;
        (0, logger_js_1.info)('Gemini', `Reconnecting in ${delay}ms (attempt ${this.retryCount}/${this.maxRetries})`);
