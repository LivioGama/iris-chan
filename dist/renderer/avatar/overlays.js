...
function applyOverlays(vrm, time, getSpeakingVolume) {
    ...
    for (const side of ['right', 'left']) {
        ...
        for (const finger of ['Index', 'Middle', 'Ring', 'Little']) {
            ...
            if (prox)
                prox.rotation.z = sign * fc;
            if (inter)
                inter.rotation.z = sign * fc * 1.1;
            if (dist)
                dist.rotation.z = sign * fc * 0.8;
        }
        ...
        if (tMeta)
            tMeta.rotation.z = sign * 0.15;
        if (tProx)
            tProx.rotation.z = sign * (0.1 + Math.sin(time * 0.3) * 0.05);
        if (tDist)
            tDist.rotation.z = sign * 0.1;
    }
    // Expressions
