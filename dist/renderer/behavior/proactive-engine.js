...
class ProactiveEngine {
    ...
    async start() {
        ...
        if (!this.apiKey || this.apiKey === 'YOUR_API_KEY_HERE') {
            (0, logger_js_1.info)('Proactive', 'Skipping proactive engine start because no API key is configured');
            return;
        }
        this.interval = setInterval(() => {
            this.tick().catch((err) => (0, logger_js_1.error)('Proactive', `Tick failed: ${err?.message || err}`));
        }, PROACTIVE_POLL_MS);
    }
    stop() {
        if (this.interval) {
    ...
    async tick() {
        if (this.inFlight)
            return;
        if (!this.voice?.canEvaluateProactively?.())
            return;
        if (this.voice?.hasPendingReplySession?.())
            return;
        ...
        if (!capture?.data || captureAgeMs > SCREEN_FRESH_MS)
            return;
        ...
        try {
            ...
            if (!payload.suggestion)
                return;
            this.eventBus.emitEvent(event_types_web_js_1.EVENT_TYPES.PROACTIVE_SUGGESTION, payload, 'proactive-engine');
    ...
    async _evaluateReplyOpportunity({ frontmostApp, capture, contextFingerprint }) {
        ...
        try {
            ...
            (0, logger_js_1.error)('ReplyAssistant', `Detection failed: ${err?.message || err}`);
            return false;
        }
        if (!analysis?.ok)
            return false;
        ...
        if (!analysis.shouldOffer && !analysis.askToHelp)
            return false;
        ...
        this.eventBus.emitEvent(event_types_web_js_1.EVENT_TYPES.PROACTIVE_SUGGESTION, replyPayload, 'reply-assistant');
    ...
    async _evaluateWithFlash({ frontmostApp, capture, contextFingerprint }) {
        ...
        try {
            ...
            if (!resp.ok) {
                (0, logger_js_1.error)('Proactive', `Flash request failed: HTTP ${resp.status}`);
                return null;
            }
            ...
            (0, logger_js_1.error)('Proactive', `Flash evaluation error: ${err?.message || err}`);
            return null;
        }
    }
    async _generateReplySuggestions({ frontmostApp, capture, analysis }) {
        if (!this.apiKey || !capture?.data)
            return [];
        const prompt = [
        ...
        try {
            ...
            if (!resp.ok) {
                (0, logger_js_1.error)('ReplyAssistant', `Draft generation failed: HTTP ${resp.status}`);
                return [];
            }
            const data = await resp.json();
            const text = data?.candidates?.[0]?.content?.parts?.[0]?.text || '';
            ...
            (0, logger_js_1.error)('ReplyAssistant', `Draft generation error: ${err?.message || err}`);
            return [];
        }
    }
}
