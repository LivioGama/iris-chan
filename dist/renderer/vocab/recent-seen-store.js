...
exports.configureRecentSeenStore = configureRecentSeenStore;
exports.setRecentSeenKnownTerms = setRecentSeenKnownTerms;
exports.addRecentSeenTerms = addRecentSeenTerms;
exports.listRecentSeenTerms = listRecentSeenTerms;
exports.buildRecentSeenPrompt = buildRecentSeenPrompt;
exports.findRecentSeenRewrite = findRecentSeenRewrite;
exports.clearRecentSeenTerms = clearRecentSeenTerms;
exports.normalizeTerm = normalizeTerm;
const learner_js_1 = require("./learner.js");
...
function configureRecentSeenStore(config = {}) {
    storeConfig = {
        ...storeConfig,
        ...(config || {}),
    };
...
function setRecentSeenKnownTerms({ vocabTerms = [], hotTerms = [], coreTerms = [], corrections = {} } = {}) {
    knownTerms = new Set([...vocabTerms, ...hotTerms, ...coreTerms]
        .map((term) => normalizeTerm(term))
        .filter(Boolean));
    knownCorrections = new Set(Object.entries(corrections || {})
        .flatMap(([wrong, right]) => [normalizeTerm(wrong), normalizeTerm(right)])
        .filter(Boolean));
    pruneExpired();
}
function addRecentSeenTerms(terms = [], context = {}) {
    ...
    for (const item of terms) {
        ...
        if (!normalizedTerm)
            continue;
        if (shouldSkipTerm(term, normalizedTerm))
            continue;
        const confidence = clampConfidence(typeof item === 'object' ? item?.confidence : null);
        if (confidence < storeConfig.minConfidence)
            continue;
...
function listRecentSeenTerms(now = Date.now()) {
    pruneExpired(now);
    return [...recentTerms];
}
function buildRecentSeenPrompt(now = Date.now()) {
...
function findRecentSeenRewrite(text, now = Date.now()) {
    ...
    for (const entry of active) {
        ...
        const match = (0, learner_js_1.findBestNgram)(input, entry.term, termWords);
        if (!match?.ngram)
            continue;
        ...
        const threshold = exact
            ? 0
            : Math.max(1, Math.min(storeConfig.rewriteDistance, Math.ceil(entry.normalizedTerm.length * 0.35)));
        if (!exact && match.distance > threshold)
            continue;
...
function shouldSkipTerm(term, normalizedTerm) {
    ...
    if (normalizedTerm.length < 3)
        return true;
    if (!/[A-Z0-9]/.test(term) && !/[^a-z\s]/i.test(term))
        return true;
    if (!/[A-Za-z]/.test(term))
        return true;
    if (COMMON_TERMS.has(normalizedTerm))
        return true;
    if (knownTerms.has(normalizedTerm))
        return true;
    if (knownCorrections.has(normalizedTerm))
        return true;
    return false;
}
...
function isClearWinner(best, second) {
    ...
    if (best.exact !== second.exact)
        return true;
    if (best.entry.normalizedTerm === second.entry.normalizedTerm)
        return true;
