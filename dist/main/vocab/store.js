...
// Ensure ~/.iris/ exists
try {
    fs.mkdirSync(config.paths.irisDir, { recursive: true });
}
catch { }
...
function syncFromSource() {
    try {
        ...
        let dest;
        try {
            dest = JSON.parse(fs.readFileSync(vocabPath, 'utf-8'));
        }
        catch {
            dest = { terms: [] };
        }
        let changed = false;
        if (src.corrections && !dest.corrections) {
            dest.corrections = src.corrections;
            changed = true;
        }
        ...
        if (changed)
            fs.writeFileSync(vocabPath, JSON.stringify(dest, null, '\t') + '\n', 'utf-8');
    }
    catch { }
}
function loadTerms() {
...
function loadHot() {
    try {
        return JSON.parse(fs.readFileSync(hotVocabPath, 'utf-8'));
    }
    catch {
        return {};
    }
...
function saveHot(hot) {
    try {
        fs.writeFileSync(hotVocabPath, JSON.stringify(hot, null, '\t') + '\n', 'utf-8');
    }
    catch { }
}
function loadStats() {
    try {
        return JSON.parse(fs.readFileSync(vocabStatsPath, 'utf-8'));
    }
    catch {
        return {};
    }
...
function trackTerms(terms) {
    ...
    for (const term of terms) {
        if (!stats[term])
            stats[term] = { count: 0, lastUsed: null };
        stats[term].count++;
        stats[term].lastUsed = now;
    }
    try {
        fs.writeFileSync(vocabStatsPath, JSON.stringify(stats, null, '\t') + '\n', 'utf-8');
    }
    catch { }
}
