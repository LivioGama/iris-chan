...
async function extractTermsWithGemini(textChunks, apiKey) {
    ...
    try {
        const resp = await fetch(`${config.gemini.flashEndpoint}?key=${apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: AbortSignal.timeout(10000) });
        if (!resp.ok)
            return [];
