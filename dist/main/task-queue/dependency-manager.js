...
function referencedByPrompt(task, candidate) {
    ...
    if (candidateId && candidateId.length >= 4 && prompt.includes(candidateId))
        return true;
    const candidateTitle = normalizeText(getTaskTitle(candidate));
    if (candidateTitle && candidateTitle.length >= 8 && prompt.includes(candidateTitle))
        return true;
...
function inferDependencies(task, allTasks = []) {
    ...
    for (const candidate of allTasks) {
        ...
        if (!candidateId || candidateId === currentId)
            continue;
        if (Number(candidate.createdAt || 0) > Number(task.createdAt || 0))
            continue;
        ...
        if (referencedByPrompt(task, candidate)
...
function unresolvedDependencies(task, taskMap) {
    ...
    for (const dependencyId of dependencies) {
        ...
        if (!TERMINAL_STATUSES.has(status) || status === 'failed' || status === 'cancelled') {
            if (status !== 'completed')
                unresolved.push(String(dependencyId));
        }
    }
    return unresolved;
}
