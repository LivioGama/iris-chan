...
`;
    if (onLog)
        onLog('[Strategy 10] Orchestrating 10 specialized sub-agents...');
    const subAgents = [];
    ...
    const results = await Promise.all(subAgents.map(async (agent) => {
        if (onLog)
            onLog(`[Strategy 10] Launching ${agent.id} (${agent.profile})...`);
        const handle = startCodingTask({
            ...
            onLog: (line) => {
                if (onLog)
                    onLog(`[${agent.id}] ${line.substring(0, 50)}...`);
            }
        });
    ...
    if (onLog)
        onLog(`[Strategy 10] ${successful.length}/10 sub-agents reported success.`);
    return {
