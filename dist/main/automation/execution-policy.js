...
class ExecutionPolicy {
    ...
    enforceSafetyContract(plan, step, executionContract) {
        if (executionContract.safetyClass === 'destructive') {
            throw makeTaskError(`Blocked destructive UI action for "${step.selector?.text || step.resultKind || step.type}". Use a dedicated verified flow instead of a generic GUI click.`, 'safety_confirmation_required');
        }
        if (executionContract.safetyClass !== 'install_cleanup') {
            return;
        }
        if (!EXPLICIT_DESTRUCTIVE_INTENT_PATTERN.test(String(plan.goal || ''))) {
            throw makeTaskError('Install cleanup requires explicit user intent before removing or ejecting installer artifacts.', 'safety_confirmation_required');
        }
    }
