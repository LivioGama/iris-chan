...
function inferAutonomousSelfImprovementPolicy(text = '') {
    ...
    if (!asksForNextSelfImprovementStep || !mentionsSelfImprovement)
        return null;
    return {
...
function inferDirectCreativeFulfillmentPolicy(text = '') {
    ...
    if (!asksForPoem && !asksForOtherCreative)
        return null;
    return {
        kind: 'fallback_policy',
        scope: 'machine',
...
function toolFamily(toolName = '') {
    ...
    if (['click_at', 'double_click', 'mouse_move', 'drag'].includes(name))
        return 'pointer';
    if (name === 'run_ui_task')
        return 'planner';
    if (['open_app', 'get_default_app', 'window_manage', 'finder_open_item', 'finder_select_item'].includes(name))
        return 'native';
    if (['type_text', 'press_key', 'scroll'].includes(name))
        return 'ax_dom';
    return name ? 'general' : '';
}
function deriveIntentFamily(text, tokenSet, domain) {
    if (text.includes('autonomous_continuation') || tokenSet.has('self_input'))
        return 'autonomous_continuation';
    if (tokenSet.has('self_verify') && (text.includes('autonomous_continuation') || tokenSet.has('self_input')))
        return 'autonomous_continuation';
    ...
    if (text.includes('stale_screen') || text.includes('unverified_action_claim') || text.includes('unverified_navigation_claim') || text.includes('pre_action_verification'))
        return 'action_verification';
    if (text.includes('status_bar') && (text.includes('battery_icon') || tokenSet.has('icon')))
        return 'status_bar_icon';
    if (text.includes('screen_reference') || tokenSet.has('screen'))
        return 'screen_reference';
    if (text.includes('default_browser') || text.includes('default_mail') || text.includes('default_app'))
        return 'resolve_default';
    if (tokenSet.has('pause') || tokenSet.has('play') || tokenSet.has('video'))
    ...
    if (domain === 'finder' || tokenSet.has('finder') || tokenSet.has('file') || tokenSet.has('folder'))
        return 'file_navigation';
    if (tokenSet.has('system') || tokenSet.has('settings') || tokenSet.has('utility'))
        return 'system_query';
    if (tokenSet.has('search') || tokenSet.has('query'))
        return 'search';
    if (tokenSet.has('activate'))
        return 'activation';
    if (tokenSet.has('open') || tokenSet.has('click') || tokenSet.has('select') || tokenSet.has('navigate') || tokenSet.has('link') || tokenSet.has('result'))
        return 'navigation';
...
function deriveTargetFeatures(text, tokenSet, domain) {
    ...
    if (text.includes('self_verify') || tokenSet.has('self_verify'))
        targets.push('self_verification');
    if (text.includes('self_input') || tokenSet.has('self_input'))
        targets.push('self_directed_input');
    if (text.includes('status_bar') || (tokenSet.has('status') && tokenSet.has('bar')))
        targets.push('status_bar');
    if (text.includes('battery_icon') || (tokenSet.has('battery') && tokenSet.has('icon')))
        targets.push('battery_icon');
    if (text.includes('screen_reference') || tokenSet.has('screen'))
        targets.push('screen_context');
    if (text.includes('stale_screen') || tokenSet.has('stale_screen'))
        targets.push('screen_context');
    ...
    if (text.includes('default_browser'))
        targets.push('default_browser');
    if (text.includes('default_mail'))
        targets.push('default_mail');
    ...
    if (tokenSet.has('video'))
        targets.push('video');
    if (tokenSet.has('file') || tokenSet.has('folder') || domain === 'finder')
        targets.push('file_target');
    if (tokenSet.has('tab'))
        targets.push('tab');
    if (tokenSet.has('settings'))
        targets.push('settings');
    if (tokenSet.has('browser'))
        targets.push('browser');
    if (tokenSet.has('editor') || tokenSet.has('vscode') || tokenSet.has('cursor') || tokenSet.has('zed') || tokenSet.has('textedit'))
        targets.push('editor');
    if (tokenSet.has('system') || tokenSet.has('utility'))
        targets.push('system');
...
function deriveEntityFeatures(tokens = [], targets = [], apps = []) {
    return uniqueSorted(tokens.filter((token) => {
        if (GENERIC_ACTION_TOKENS.has(token))
            return false;
        if (APP_HINT_TOKENS.has(token))
            return false;
        if (token === 'noise')
            return false;
...
function inferNoisyPointerScreenReference({ rawText = '', tokenSet, pointerMode = 'nonpointer', targets = [] } = {}) {
    if (pointerMode !== 'pointer')
        return false;
    if (targets.includes('screen_context'))
        return false;
    if (!hasNonAsciiLetters(rawText))
        return false;
    if (!tokenSet.has('screen'))
        return false;
    const rawTerms = countRawTerms(rawText);
    if (rawTerms === 0 || rawTerms > 6)
        return false;
    const asciiTokens = countAsciiTokens([...tokenSet]);
    return asciiTokens <= 1;
}
function inferFlattenedPreparationRecoveryPolicy(event = {}) {
    const rawText = String(event.guidanceText
        || event.userText
    ...
    if (!mentionsPlanner || !mentionsOpenApp || !mentionsPointer || !mentionsRecovery)
        return null;
    return {
        kind: 'fallback_policy',
        scope: 'machine',
...
class LearningManager {
    ...
    recordToolExecution(name, args, result, success, durationMs) {
        ...
        if (this.recentToolExecutions.length > 40)
    ...
    resolveToolRequest(name, args = {}) {
        ...
        if (name === 'get_default_app') {
            ...
            const appName = this.memoryStore.getValue(requested === 'mail' ? 'environment.default_mail.app_name' : 'environment.default_browser.app_name', '');
            if (appName) {
        ...
        if (!learned || !Array.isArray(sequence) || !sequence.length)
            return { name, args };
        if (sequence.some((step) => POINTER_SEQUENCE_TOOLS.has(step.name)))
            return { name, args };
    ...
    _enqueueDeferredIssue(issueSignature, event) {
        if (this.deferredIssueQueue.some((item) => item.issueSignature === issueSignature))
            return;
    ...
    _pumpDeferredIssues() {
        if (!this.deferredIssueQueue.length)
            return;
    ...
    _inferTier(toolName = '') {
        ...
        if (name === 'run_ui_task')
            return 'planner';
        if (name === 'type_text' || name === 'press_key' || name === 'scroll')
            return 'ax_dom';
        return 'general';
    }
    async _drainQueue() {
    ...
    async _applyCoreGapEvent(event) {
        ...
        if (evidence) {
            ...
            if (issue.evidence.length > 5)
                issue.evidence = issue.evidence.slice(-5);
        }
        issue.count += 1;
