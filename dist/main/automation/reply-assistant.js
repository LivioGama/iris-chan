...
function parseFrontmostApp(resultText = '') {
    ...
    if (!match)
        return { name: String(resultText || '').trim(), windows: [] };
    return {
        name: match[1].trim(),
...
function looksLikeMessageText(text = '') {
    ...
    if (value.length < 2 || value.length > 240)
        return false;
    if (COMPOSER_KEYWORDS.test(value) || SEND_KEYWORDS.test(value))
        return false;
    return true;
}
function collectSnapshotFacts(snapshot = {}) {
    ...
    for (const element of elements) {
        ...
        if (!detail)
            continue;
        if (UNREAD_KEYWORDS.test(detail))
            unreadSignal = true;
...
function inferLatestMessageAuthorship(messages = []) {
    ...
    if (!latest)
        return { latestMessage: '', latestAuthoredBySelf: false };
    return {
...
function buildContextSummary(frontmostApp, snapshot, facts, authorship) {
    ...
    if (facts.shortcutHint)
        lines.push(`Send shortcut hint: ${facts.shortcutHint}`);
    if (facts.unreadSignal)
        lines.push('Unread indicator visible');
