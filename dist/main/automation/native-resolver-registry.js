...
function isNativeEligibleStep(step = {}) {
    ...
    if (step.type === 'openUrl')
        return true;
    if (step.type === 'navigateHistory')
        return true;
    if (step.type === 'searchInCurrentContext')
        return true;
    if (step.type === 'clickSearchResult')
        return true;
    if (step.type === 'mediaControl')
        return true;
    if (step.type === 'editorCommand')
        return true;
    if ((step.type === 'clickElement' || step.type === 'selectItemByText') && isBrowserApp(step.appHint || step.appName))
        return true;
    if ((step.type === 'clickElement' || step.type === 'selectItemByText') && String(step.appHint || step.appName || '').trim() === 'Finder')
        return true;
    return false;
}
function resolverIdForStep(step = {}, appName = '') {
    if (!step || !step.type)
        return '';
    if (step.type === 'openApp') {
        if (/default (browser|mail)/i.test(step.appName || ''))
            return RESOLVER_IDS.SYSTEM_DEFAULT_APP;
        if (isEditorApp(step.appName || step.appHint || appName))
            return RESOLVER_IDS.EDITOR_ACTIVATE;
        return RESOLVER_IDS.APP_ACTIVATE;
    }
    if (step.type === 'resolveSystemDefault')
        return RESOLVER_IDS.SYSTEM_QUERY;
    if (step.type === 'openUrl')
        return RESOLVER_IDS.BROWSER_OPEN_URL;
    if (step.type === 'searchInCurrentContext')
        return RESOLVER_IDS.BROWSER_SEARCH;
    if (step.type === 'navigateHistory')
        return RESOLVER_IDS.BROWSER_HISTORY;
    if (step.type === 'clickSearchResult')
        return RESOLVER_IDS.BROWSER_RESULT;
    if (step.type === 'mediaControl')
        return RESOLVER_IDS.BROWSER_MEDIA;
    if (step.type === 'editorCommand')
        return RESOLVER_IDS.EDITOR_COMMAND;
    if (step.type === 'clickElement' && String(appName || step.appHint || step.appName || '').trim() === 'Finder')
        return RESOLVER_IDS.FINDER_OPEN;
    if (step.type === 'selectItemByText' && String(appName || step.appHint || step.appName || '').trim() === 'Finder')
        return RESOLVER_IDS.FINDER_SELECTION;
