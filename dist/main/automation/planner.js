...
function createCheckpoint(step, index, total) {
    if (index === total - 1)
        return { kind: 'final', reason: 'task-complete' };
    if (step.type === 'openApp')
        return { kind: 'app-switch', reason: 'app-opened' };
    if (step.type === 'openUrl')
        return { kind: 'navigation', reason: 'url-opened' };
    if (step.type === 'searchInCurrentContext')
        return { kind: 'search', reason: 'query-submitted' };
    if (step.type === 'clickSearchResult')
        return { kind: 'final', reason: 'result-opened' };
    if (step.type === 'mediaControl')
        return { kind: 'final', reason: 'media-control' };
    if (step.type === 'editorCommand')
        return { kind: 'final', reason: 'editor-command' };
    if (step.type === 'resolveSystemDefault')
        return { kind: 'final', reason: 'system-query' };
    if (step.type === 'cleanupInstallArtifact')
        return { kind: 'final', reason: 'install-cleanup' };
    return null;
}
