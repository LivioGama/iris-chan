...
function inferHireableFlag(args = {}, lane = normalizeExecutionLane(args.execution_lane || args.executionLane || args.lane)) {
    if (typeof args.hireable === 'boolean')
        return args.hireable;
...
class SelfImprovementManager {
    ...
    findMatchingSkill({ goal = '', appHint = '' } = {}) {
        ...
        for (const entry of registry.skills) {
            if (!entry || entry.status !== 'active')
                continue;
            if (entry.stability && entry.stability !== 'stable')
                continue;
            if (entry.preferredExecutionPath?.plan && !planLikelySatisfiesGoal(goal, entry.preferredExecutionPath?.plan || {}))
                continue;
            const appNames = Array.isArray(entry.match?.appNames) ? entry.match.appNames.map(normalizeText) : [];
            if (appNames.length && normalizedApp && !appNames.includes(normalizedApp))
                continue;
            let score = 0;
            for (const intent of entry.match?.intents || []) {
                if (intent && normalizedGoal.includes(normalizeText(intent)))
                    score += 5;
            }
            for (const keyword of entry.match?.keywords || []) {
                if (keyword && normalizedGoal.includes(normalizeText(keyword)))
                    score += 1;
            }
            if (appNames.length && normalizedApp && appNames.includes(normalizedApp))
                score += 3;
    ...
    recordSuccessfulPlan({ goal = '', appHint = '', plan = null, usedLearnedSkill = false, recoveredFromSkillFailure = false } = {}) {
        if (!plan || !Array.isArray(plan.steps) || plan.steps.length < 2 || usedLearnedSkill)
            return null;
        const registry = this.loadRegistry();
        ...
        this.saveRegistry(registry);
        if (pattern.learnedSkillId)
            return null;
        if (pattern.count < REPEAT_THRESHOLD && !recoveredFromSkillFailure)
            return null;
