...
function looksLikeGenericPointerCorrection(text = '') {
    ...
    if (/\b(click|open|tap|press|select|move|drag)\b.{0,20}\b(there|here|that|this|it)\b/.test(normalized))
        return true;
    if (/\b(move|drag)\b.{0,20}\b(mouse|cursor|pointer)\b/.test(normalized))
        return true;
    if (/\b(make your move|move your move|your move)\b/.test(normalized))
        return true;
...
function isScreenVisibilityQuestion(text = '') {
    ...
    if (!normalized)
        return false;
    if (!/\b(screen|screenshot|display|monitor)\b/.test(normalized))
        return false;
    if (/\b(click|open|tap|select|press|drag|scroll|type)\b/.test(normalized))
        return false;
    if (/\b(there|here|that|this)\b/.test(normalized))
        return false;
...
function isActionVerificationGuidance(text = '') {
    ...
    return ((mentionsVerification && (mentionsFalseClaim || mentionsStaleScreen || mentionsPointerAction))
...
function needsPreparationBeforeClick(text = '') {
    ...
    if (!normalized)
        return false;
    return (/\bbefore (?:you )?(?:click|right click|double click)\b/.test(normalized)
...
function isScreenReferentialGoal(text = '') {
    ...
    if (!normalized)
        return false;
    if (/\b(click|open|tap|press|select)\b.{0,20}\b(there|here|that|this|it)\b/.test(normalized))
        return true;
    if (/\b(where i (?:said|told you|pointed)|the one i (?:said|told you))\b/.test(normalized))
        return true;
    if (/\b(written there|what is written there|that channel|that link|that button|that result)\b/.test(normalized))
        return true;
    return false;
}
function isAutonomousContinuationGuidance(text = '') {
    ...
    return (((mentionsContinue || mentionsAbsenceWindow) && (mentionsSelfVerification || mentionsSelfInput || mentionsAutonomy || mentionsBacklogClearance))
...
function isEditorSelfImprovementGeneralizationGuidance(text = '') {
    ...
    if (!normalized)
        return false;
    const mentionsRepeatedTeaching = (/\b(keep telling you|keep teaching you|ive just taught you|i've just taught you|stop asking|same guidance|every time)\b/.test(normalized)
    ...
    const mentionsNoNeedToAskAgain = (/\b(?:dont|don't|do not|no need to)\b.{0,20}\b(?:ask|re-ask|reask|repeat|clarify|question)\b/.test(normalized)
...
function isThoroughExecutionGuidance(text = '') {
    ...
    if (!normalized)
        return false;
    return (/\b(do it properly|do it well|do it right|do it thoroughly|do it carefully|be more thorough|be thorough|be more decisive|handle it properly|properly this time|just do it|do it already)\b/.test(normalized)
...
function isConflictResolutionContinuationGuidance(text = '') {
    ...
    const mentionsLightTouchReview = (/\b(without|dont|don't|avoid|stop)\b.{0,24}\b(over[- ]?read(?:ing)?|overthink(?:ing)?|reading too much|digging too deep|deep reading)\b/.test(normalized)
...
class LearningClassifier {
    ...
    classifyRecovery({ failedTools = [], successfulTools = [], latestUserText = '' } = {}) {
        if (!failedTools.length || !successfulTools.length)
            return null;
