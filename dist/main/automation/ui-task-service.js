...
function classifyOpenAppDomain(appName = '', resolverId = '') {
    if (resolverId === 'editor.activate')
        return 'editor';
    if (resolverId === 'system.default_app')
        return 'system';
...
class UITaskService extends EventEmitter {
    ...
    stopActiveTask(reason = 'User interrupted the task') {
        if (!this.activeTask)
            return { ok: true, result: 'No active UI task' };
    ...
    async _executeStepWithContract({ taskId, plan, step, executionContract, signal }) {
        ...
        try {
            ...
            if (executionContract.primaryTier === 'tars') {
                ...
                if (err.tarsRescue)
                    gatedError.tarsRescue = err.tarsRescue;
                throw gatedError;
            }
    ...
    async _executeClickByText(step, signal) {
        ...
        if (this.browserAdapter.isSupported(appName)) {
            ...
            if (browserResult.code === 'ambiguous') {
                throw this._makePointerEligibleError(`Multiple matches for "${targetText}": ${(browserResult.matches || []).join(', ')}`, 'ambiguous', { matches: browserResult.matches || [] });
            }
        }
        ...
        if (parsed?.ambiguous) {
            throw this._makePointerEligibleError(`Multiple matches for "${targetText}": ${(parsed.matches || []).join(', ')}`, 'ambiguous', { matches: parsed.matches || [] });
        }
