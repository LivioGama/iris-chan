    ...
    for (const skill of scan()) {
        ...
        return (args) => new Promise((resolve) => {
            ...
            child.on('close', (code, signal) => {
                ...
                try {
                    fs.appendFileSync(SKILL_LOG, logLines.join('\n') + '\n');
                }
                catch { }
                ...
                try {
                    ...
                    resolve({ ok: true, result: stdout.trim().slice(0, 500) });
                }
            });
            child.on('error', (err) => {
                activeSkillProcess = null;
...
function getSkillContent(name) {
    ...
    if (!skill)
        return { ok: false, result: `Skill "${name}" not found. Available: ${all.map(s => s.name).join(', ')}` };
