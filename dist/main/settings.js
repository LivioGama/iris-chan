...
function resolveVoicePreset(name = '') {
    ...
    return VOICE_PRESETS.find((preset) => {
        if (normalizePresetName(preset.name) === target)
            return true;
...
function migrateLegacyLogging(rawSettings) {
    if (!fs.existsSync(LEGACY_LOG_SETTINGS_PATH))
        return rawSettings;
    try {
    ...
    for (const namespace of Object.keys(REGISTRY)) {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(result)) {
                if (['applied', 'liveApply', 'restartRequired'].includes(key))
                    continue;
...
function startWatcher() {
    ...
    try {
        ...
        watcher = fs.watch(settingsDir, (_eventType, filename) => {
            if (filename && String(filename) !== settingsFileName)
                return;
            if (writing) {
...
function registerApplyHandler(namespace, handler) {
    ...
    if (typeof handler !== 'function')
        throw new Error(`Settings apply handler for ${namespace} must be a function`);
...
function getMetadata() {
    return {
        path: SETTINGS_PATH,
        registry: deepClone(Object.fromEntries(Object.entries(REGISTRY).map(([namespace, value]) => [
            namespace,
            {
