...
function logSkillOutput(label, command, stdout, stderr, err) {
    ...
    try {
        fs.appendFileSync(SKILL_LOG, lines.join('\n') + '\n');
    }
    catch { }
}
...
function derivePatchFromRequest(request = '') {
    ...
    if (/\bdirect mode\b.*\bon\b/i.test(text))
        setPath(patch, 'behavior.directMode', true);
    if (/\bdirect mode\b.*\boff\b/i.test(text))
        setPath(patch, 'behavior.directMode', false);
    if (/\bfeedback mode\b.*\bon\b/i.test(text))
        setPath(patch, 'behavior.feedbackEnabled', true);
    if (/\bfeedback mode\b.*\boff\b/i.test(text))
        setPath(patch, 'behavior.feedbackEnabled', false);
    if (/\bintroversion mode\b.*\bon\b/i.test(text))
        setPath(patch, 'behavior.introversionEnabled', true);
    if (/\bintroversion mode\b.*\boff\b/i.test(text))
        setPath(patch, 'behavior.introversionEnabled', false);
...
function buildRoutingRegistrySummary() {
    ...
    return Object.fromEntries(Object.entries(metadata.registry || {}).map(([namespace, value]) => [
        namespace,
        {
...
async function groqPatchFromRequest(request = '') {
    if (!process.env.GROQ_API_KEY || !String(request || '').trim() || typeof fetch !== 'function')
        return null;
...
async function route_request(args = {}) {
    ...
    if (!request)
        return { ok: false, result: 'Routing unavailable right now: empty request.' };
...
async function run_terminal_command(args) {
    ...
    if (!command)
        return { ok: false, result: 'No command provided' };
