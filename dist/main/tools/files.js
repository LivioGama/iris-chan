...
async function move_file(args) {
    ...
    if (!args.source && !args.from)
        return { ok: false, result: 'No source path provided' };
    if (!args.destination && !args.to)
        return { ok: false, result: 'No destination path provided' };
    try {
...
async function finder_select_item(args) {
    ...
    if (!name)
        return { ok: false, result: 'No Finder item name provided' };
    try {
...
async function resolve_install_cleanup_target(args = {}) {
    ...
    if (explicitPath) {
        ...
        if (!target)
            return { ok: false, result: `Refusing to clean up non-installer target: ${explicitPath}` };
        if (!fs.existsSync(target.path))
            return { ok: false, result: `Cleanup target does not exist: ${target.path}` };
    ...
    if (explicitName) {
        try {
            ...
            if (!target)
                return { ok: false, result: `Refusing to clean up non-installer Finder target: ${finderPath}` };
            return { ok: true, ...target };
        }
        catch (err) {
    ...
    if (!target)
        return { ok: false, result: `Refusing to clean up non-installer selection: ${selectedPath[0]}` };
    return { ok: true, ...target };
}
...
function _runAppleScript(script, timeout = 5000) {
    ...
    if (r.status !== 0 || r.error)
        throw new Error(r.stderr || r.error?.message || 'AppleScript failed');
...
function _getBrowserUrl(appName) {
    ...
    for (const script of strategies) {
        try {
            return _runAppleScript(script, 3000);
        }
        catch { }
    }
    return null;
...
function _downloadViaUrlFallback(appName, savePath) {
    ...
    try {
        ...
        if (!imgs.length)
            return { ok: false, result: 'No images found on page' };
        ...
        if (!fs.existsSync(savePath))
            return { ok: false, result: 'Download failed' };
...
async function download_browser_image(args) {
    ...
    try {
        const appResult = _runAppleScript('tell application "System Events" to get name of first application process whose frontmost is true', 3000);
        let jsResult;
        const jsCode = BROWSER_JS.replace(/"/g, '\\"');
        if (appResult === 'Safari') {
            jsResult = _runAppleScript(`tell application "Safari" to do JavaScript "${jsCode}" in current tab of front window`);
        }
        else {
            try {
                jsResult = _runAppleScript(`tell application "${appResult}" to execute front window's active tab javascript "${jsCode}"`);
            }
            catch {
