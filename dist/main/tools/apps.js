...
`.trim();
    ...
    if (proc.error)
        throw proc.error;
    if (proc.status !== 0)
        throw new Error((proc.stderr || 'python3 failed').trim());
    return (proc.stdout || '').trim();
}
...
function resolveDefaultApp(name = '') {
    ...
    if (!bundleId)
        return { ok: false, error: `Could not resolve default ${kind}` };
...
async function open_app(args) {
    ...
    if (isDefaultAppRequest(name)) {
        try {
            ...
            if (!resolved.ok)
                return { ok: false, result: resolved.error || `Could not resolve ${name}` };
