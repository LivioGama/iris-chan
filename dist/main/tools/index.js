...
function summarizeRuntimeLogEvidence(lines = []) {
    ...
    const entries = [
        ...
        },
        {
            ...
            summarize(matchLines) {
                const reasons = [...new Set(matchLines
...
function enrichArgsWithScientificRuntimeEvidence(name, args = {}) {
    if (!SCIENTIFIC_TASK_TOOLS.has(name))
        return args || {};
...
function splitTaskIds(value = '') {
    return Array.from(new Set(Array.from(String(value || '').matchAll(TASK_ID_PATTERN))
...
function parseManagementCorrections(text = '') {
    ...
    for (const match of rawText.matchAll(/([a-z][a-z0-9_-]*eng)\s*\(([^)]+)\)/ig)) {
        ...
        const uniqueTaskIds = taskIds.filter((taskId) => {
            ...
            if (seenApprovalIds.has(composite))
                return false;
...
function attachManagementCorrections(name, args = {}) {
    if (!MANAGEMENT_CORRECTION_TOOLS.has(name) || !args || typeof args !== 'object')
        return args;
    const description = typeof args.description === 'string' ? args.description : '';
    if (!description || description.includes(MANAGEMENT_CORRECTION_MARKER))
        return args;
...
function inferUserIntentText(name, args = {}) {
    ...
    if (typeof args.goal === 'string' && args.goal.trim())
        return normalizeWhitespace(args.goal);
    if (typeof args.user_intent === 'string' && args.user_intent.trim())
        return normalizeWhitespace(args.user_intent);
    if (typeof args.description === 'string' && args.description.trim())
        return normalizeWhitespace(args.description);
    if (name === 'open_app')
        return normalizeWhitespace(args.name || '');
    return '';
}
function shouldPromoteToSemanticUiTool(name, args = {}) {
    if (!ENABLE_SEMANTIC_UI_FAST_PATH)
        return false;
    if (!UI_SEMANTIC_TOOLS.has(name) || name === DIRECT_UI_TOOL)
        return false;
    const intent = inferUserIntentText(name, args);
    if (!intent || intent.length < 12)
        return false;
    if (!SEMANTIC_UI_INTENT_PATTERN.test(intent))
        return false;
    if (EXPLICIT_LOW_LEVEL_PATTERN.test(intent))
        return false;
...
function buildScientificSelfReview(name, args, result, durationMs, policy) {
    if (!ENABLE_SCIENTIFIC_SELF_REVIEW)
        return null;
    if (!SCIENTIFIC_TASK_TOOLS.has(name) || args?.self_review === false)
        return null;
    const summary = {
        tool: name,
        durationMs,
...
async function executeCore(name, args, markOk) {
    ...
    if (name === 'set_workspace') {
        ...
        if (!dir)
            return { ok: false, result: 'No directory provided' };
