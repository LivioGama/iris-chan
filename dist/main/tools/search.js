...
function rememberProvider(provider, memoryStore = getMemoryStore()) {
    if (!memoryStore?.upsert || !provider)
        return;
    memoryStore.upsert({
...
async function emitSearchRuntimeEvent(payload) {
    ...
    if (!convexClient?.saveRuntimeEvent)
        return;
...
async function runSearchWithProvider(provider, query, env = process.env) {
    if (provider === PROVIDER_PERPLEXITY)
        return runPerplexitySearch(query, env);
    if (provider === PROVIDER_OLLAMA)
        return runOllamaSearch(query, env);
    if (provider === PROVIDER_CHATGPT_APP)
        return runChatgptAppSearch(query);
    throw new Error(`Unsupported search provider: ${provider}`);
}
async function web_search(args = {}) {
    const query = String(args.query || '').trim();
    if (!query)
        return { ok: false, result: 'No query provided' };
...
async function ask_chatgpt(args = {}) {
    ...
    if (!prompt)
        return { ok: false, result: 'No prompt provided' };
    try {
