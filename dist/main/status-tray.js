...
function createTrayController({ electron, avatarWindowModule, kanbanWindowModule, logModule, } = {}) {
    ...
    function showWindow(win, { focus = false } = {}) {
        ...
        if (!isWindowVisible(win)) {
            if (!focus && typeof win.showInactive === 'function')
                win.showInactive();
            else
                win.show?.();
        }
    ...
    function hideWindow(win) {
        if (!win || isWindowDestroyed(win) || !isWindowVisible(win))
            return false;
        win.hide?.();
        return true;
    }
    ...
    function bindWindowVisibility(win) {
        if (!win || isWindowDestroyed(win) || typeof win.on !== 'function' || win.__irisTrayBound)
            return;
